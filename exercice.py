...
def main():
	print([int(b) for b in struct.pack("h", 256)])
	print(struct.unpack("h", bytes([2, 1])))

	print(merge_channels([[11, 12], [12, 22]]))
	...
	with wave.open("data/stravinsky.wav", "rb") as reader:
		data = reader.readframes(int(reader.getnframes()*0.3))
		samples = convert_to_samples(data)
		samples = [s * 0.2 for s in samples]
		data = convert_to_bytes(samples)
		with wave.open("data/stravinsky.wav", "wb") as writer:
			writer.setnchannels(2)
			writer.setsampwidth(2)
			writer.setframerate(SAMPLING_FREQ)
			writer.writeframes(data)
